...
class DataHolder():
    def __init__(self, data, targets, tokenizer, target_transforms=lambda x:x, opener=lambda x: Image.open(x).convert('RGB'), chunk_size=512):
        self.data = data
    ...
    def __getitem__(self, idx):
        ...
        if chunk_size <= 0:
            print('Error with chunksize', self.targets[idx], chunk_size)
            print(elt)
            chunked_elt = torch.zeros(1, 1)
...
if __name__ == '__main__':
    ...
    MAX_BATCH_SIZE = 5000
    if args.text_is_gpt3:
        opener = lambda x:clean(json.load(open(x))['choices'][0]['text'])
    else:
        opener = lambda x:clean(open(x).read())

    targets = os.listdir(args.text_path)
    text_folder = [os.path.join(args.text_path, t) for t in targets]
    ...
    dataset = DataHolder(text_folder, targets, tokenizer=tokenizer, opener=opener, chunk_size=512)
    ...
    if args.save_features != '':
        torch.save(features, f'{args.save_features}_with_tokens{args.return_tokens}.pt')
